/* Agent tool registry — generic core + structured tools (U4/U5/U6). */
...
const STRUCTURED_TOOLS = [
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        item: { description: 'Object to append' },
      },
  ...
  {
    name: 'publish',
    description: 'Publish the current draft to the live site. Strips API keys from the public copy.',
...
function execAddItem(a, session) {
  ...
  const check = validateNewItem(a.collection, a.item || {});
  if (!check.ok) return check;
  let next = col.arr.concat([deepClone(a.item)]);
...
async function executeTool(name, args, ctx) {
  ...
  if (name === 'setContentPath') {
    ...
    const result = session.setPath(a.path, a.value);
