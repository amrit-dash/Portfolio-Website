...
const VIBE_PRESET_COUNTS = schema.VIBES.length + ' built-in vibe presets (' + schema.getVisibleVibes().length + ' core visible · ' + schema.getExtendedVibes().length + ' extended hidden in admin)';
...
const BASE_SYSTEM = [
  ...
  'Structured tools (addItem, removeItem, reorder, applyVibePreset, applyCustomVibe, saveCustomVibe, deleteCustomVibe, listCustomVibes, readCustomVibe, readAppearanceConfig, updateAppearance, setProjectImage, setCv) handle arrays and presets safely.',
  'Appearance lives under cosmetics.* — animated wallpapers (circuits, waves, aurora, cosmos, matrixrain, particles, lightning, rain, thunderstorm, binarystream, nebula, morphgeo, fluidcore, honeycombGlow, snowinteractive, ripplepool, fireflies), static patterns (3dgrid, honeycomb, padgrid), honeycombStyle (outline|fill), honeycombGlowDensity (honeycombGlow max simultaneous glow), interactive params (cursorInteractStrength, cursorTrailLength, cursorParticleDensity, cursorSweepRadius), global cursorEffect (none|trail|comet|ripple|spark|glow) with trail/comet/ripple sub-fields (cursorEffectTrailStyle, cursorEffectTrailLength, cursorEffectIntensity, cursorEffectRippleCount, cursorEffectRippleSpeed, cursorEffectCometDirection, cursorEffectCometIntensity, cursorEffectCometSpeed), cursorRingLag (ring/trail cursor follow speed), uiGlassOpacity (panel/window translucency), wallpaperColor + wallpaperUseAccent (multi-tone looks), dual wallpaper cosmetics.wallpaper2 (enabled · bgPattern · per-layer brightness/intensity/animSpeed/animPaused/randomness/tint + pattern params — read readAppearanceConfig.patternCompatibility before pairing; dual-bg only on multi-tone + gridstorm/phosphorgrid/synthwave; use thunderstorm instead of rain+lightning dual), wallpaperAnimSpeed (global canvas/CSS motion), wallpaperRandomness, vignetteIntensity/Direction, ' + VIBE_PRESET_COUNTS + ', unlimited customVibes slots (custom-* ids). Read: readAppearanceConfig. Write: updateAppearance (batch), setContentPath (single field or cosmetics.wallpaper2.* leaf), applyVibePreset / applyCustomVibe, saveCustomVibe, deleteCustomVibe. When creating custom vibes (saveCustomVibe or updateAppearance+saveToCustom), always set a short human name inferred from cosmetics — theme, bgPattern, accent/wallpaper colors, cursorEffect (e.g. dark+matrixrain → "Neon Matrix", light+snowinteractive → "Winter Drift", split accent+wallpaper → "Coral Split"); never leave new slots unnamed.',
